...
CREATE TABLE  ciclo (codigo int  NOT NULL, anho varchar(50) NOT NULL, numero int NOT NULL, fecha_inicio varchar(50) DEFAULT NULL, fecha_final varchar(50) DEFAULT NULL, ES_DEFAULT int) tablespace system;
...
CREATE OR REPLACE FUNCTION BUSCAR_CURSO_carrera_ciclo(carrera IN CURSO.CARRERA_ID%TYPE, ciclo IN CURSO.CICLO_ID%TYPE)
RETURN Types.ref_cursor 
AS 
        curso_cursor types.ref_cursor; 
BEGIN 
  OPEN curso_cursor FOR 
       SELECT CODIGO, NOMBRE, CREDITOS, HORAS_SEMANALES,CARRERA_ID,CICLO_ID FROM CURSO WHERE CARRERA_ID=carrera AND CICLO_ID=ciclo;
RETURN curso_cursor; 
END;
/
...
/
---BUSCAR
CREATE OR REPLACE FUNCTION BUSCAR_GRUPO(CICLO_ID IN GRUPO.CICLO_ID%TYPE, cursobuscar IN GRUPO.CURSO_ID%TYPE)
RETURN Types.ref_cursor 
AS 
        grupo_cursor types.ref_cursor; 
BEGIN 
  OPEN grupo_cursor FOR 
       SELECT CODIGO,CICLO_ID,NUM_GRUPO,CURSO_ID,HORARIO,PROFESOR_ID FROM GRUPO WHERE CICLO_ID=CICLO_ID AND CURSO_ID=cursobuscar; 
RETURN grupo_cursor; 
END;
/

CREATE OR REPLACE FUNCTION BUSCAR_GRUPO_Profesor(CICLO_ID IN GRUPO.CICLO_ID%TYPE, PROFESOR_ID IN GRUPO.PROFESOR_ID%TYPE)
RETURN Types.ref_cursor 
AS 
        grupo_cursor types.ref_cursor; 
BEGIN 
  OPEN grupo_cursor FOR 
       SELECT CODIGO,CICLO_ID,NUM_GRUPO,CURSO_ID,HORARIO,PROFESOR_ID FROM GRUPO WHERE CICLO_ID=CICLO_ID AND PROFESOR_ID=PROFESOR_ID; 
RETURN grupo_cursor; 
END;
/
---LISTAR
...
CREATE OR REPLACE FUNCTION BUSCAR_ALUMNO_CARRERA(idbuscar IN ALUMNO.carrera_id%TYPE)
RETURN Types.ref_cursor 
AS 
        alumno_cursor types.ref_cursor; 
BEGIN 
  OPEN alumno_cursor FOR 
       SELECT CEDULA, USUARIO_ID, NOMBRE,TELEFONO,EMAIL, FECHA_NACIMIENTO, carrera_id FROM ALUMNO WHERE carrera_id=idbuscar;
RETURN alumno_cursor; 
END;
/
...
BEGIN 
  OPEN alumno_cursor FOR 
       SELECT CEDULA, USUARIO_ID, NOMBRE,TELEFONO,EMAIL, FECHA_NACIMIENTO, carrera_id FROM ALUMNO; 
RETURN alumno_cursor; 
END;
/
---BORRAR 
...
----------------- TABLA CICLO ----------------- 
---INSERTAR
CREATE OR REPLACE PROCEDURE INSERTAR_CICLO(CODIGO IN CICLO.CODIGO%TYPE, ANHO IN CICLO.ANHO%TYPE, NUMERO IN CICLO.NUMERO%TYPE, FECHA_INICIO IN CICLO.FECHA_INICIO%TYPE, FECHA_FINAL IN CICLO.FECHA_FINAL%TYPE, ES_DEFAULT IN CICLO.ES_DEFAULT%TYPE)
AS 
BEGIN
INSERT INTO CICLO VALUES(CODIGO, ANHO, NUMERO, FECHA_INICIO, FECHA_FINAL, ES_DEFAULT);

END;
/
---MODIFICAR
CREATE OR REPLACE PROCEDURE MODIFICAR_CICLO (CODIGO IN CICLO.CODIGO%TYPE, ANHO IN CICLO.ANHO%TYPE, NUMERO IN CICLO.NUMERO%TYPE, FECHA_INICIO IN CICLO.FECHA_INICIO%TYPE, FECHA_FINAL IN CICLO.FECHA_FINAL%TYPE, ES_DEFAULT IN CICLO.ES_DEFAULT%TYPE)
AS
BEGIN
UPDATE CICLO SET ANHO=ANHO, NUMERO=NUMERO, FECHA_INICIO=FECHA_INICIO, FECHA_FINAL=FECHA_FINAL, ES_DEFAULT=ES_DEFAULT WHERE CODIGO=CODIGO;
END;
/
---BUSCAR
CREATE OR REPLACE FUNCTION BUSCAR_CICLO(idbuscar IN CICLO.ANHO%TYPE)
RETURN Types.ref_cursor 
AS 
        ciclo_cursor types.ref_cursor; 
BEGIN 
  OPEN ciclo_cursor FOR 
       SELECT CODIGO, ANHO, NUMERO, FECHA_INICIO, FECHA_FINAL, ES_DEFAULT FROM CICLO WHERE ANHO=idbuscar; 
RETURN ciclo_cursor; 
END;
/

CREATE OR REPLACE FUNCTION BUSCAR_CICLO_Codigo(idbuscar IN CICLO.CODIGO%TYPE)
RETURN Types.ref_cursor 
AS 
        ciclo_cursor types.ref_cursor; 
BEGIN 
  OPEN ciclo_cursor FOR 
       SELECT CODIGO, ANHO, NUMERO, FECHA_INICIO, FECHA_FINAL, ES_DEFAULT FROM CICLO WHERE CODIGO=idbuscar; 
RETURN ciclo_cursor; 
END;
/
...
BEGIN 
  OPEN ciclo_cursor FOR 
       SELECT CODIGO, ANHO, NUMERO, FECHA_INICIO, FECHA_FINAL, ES_DEFAULT FROM CICLO; 
RETURN ciclo_cursor; 
END;
/
---BORRAR 
...
CREATE OR REPLACE FUNCTION BUSCAR_MATRICULA_ID(ALUMNO_ID IN MATRICULA.ALUMNO_ID%TYPE)
RETURN Types.ref_cursor 
AS 
        matricula_cursor types.ref_cursor; 
BEGIN 
...
CREATE OR REPLACE FUNCTION BUSCAR_MATRICULA_GRUPO(GRUPO_ID IN MATRICULA.GRUPO_NUM%TYPE)
RETURN Types.ref_cursor 
AS 
        matricula_cursor types.ref_cursor; 
BEGIN 
  OPEN matricula_cursor FOR 
       SELECT ALUMNO_ID, GRUPO_NUM, CURSO_ID, ESTADO_ID, NOTA FROM MATRICULA WHERE GRUPO_NUM=GRUPO_ID;
RETURN matricula_cursor; 
END;
/
